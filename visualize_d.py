...
def plot_metric_alpha(dict_0, dict_1, dict_2, task_nums=10, bt=False, metric_name='avg per task acc'):
    ...
    plt.plot(np.array(task_range) + 1, max_data_0, '-*', label=f'd=8e-3 Pareto', color=curve_colors[0],
             markersize=8)
    plt.plot(np.array(task_range) + 1, mean_data_0, '--', label=f'd=8e-3 mean', color=curve_colors[0])
    plt.fill_between(np.array(task_range) + 1, max_data_0, min_data_0, facecolor=fill_colors[0], alpha=0.5,
                     label=f'd=8e-3 range')

    plt.plot(np.array(task_range) + 1, max_data_1, '-*', label=f'd=5e-3 Pareto', color=curve_colors[1],
             markersize=8)
    plt.plot(np.array(task_range) + 1, mean_data_1, '--', label=f'd=5e-3 mean', color=curve_colors[1])
    plt.fill_between(np.array(task_range) + 1, max_data_1, min_data_1, facecolor=fill_colors[1], alpha=0.5,
                     label=f'd=5e-3 range')

    plt.plot(np.array(task_range) + 1, max_data_2, '-*', label=f'd=2e-3 Pareto', color=curve_colors[2], markersize=8)
    plt.plot(np.array(task_range) + 1, mean_data_2, '--', label=f'd=2e-3 mean', color=curve_colors[2])
    plt.fill_between(np.array(task_range) + 1, max_data_2, min_data_2, facecolor=fill_colors[2], alpha=0.5,
                     label=f'd=2e-3 range')
...
if __name__ == '__main__':
    ...
    dict_all_accs_0 = torch.load(os.path.join(data_dir, f'dict_all_accs_0.99_0.008.pt'))
    dict_all_accs_1 = torch.load(os.path.join(data_dir, f'dict_all_accs_0.99_0.005.pt'))

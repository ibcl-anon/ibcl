...
def pref_convex_combination(pref: list, dict_reuse_map={}, model_type='small', num_extremes=3):
...
def gen_acc_dict(all_data_test, all_label_test, data_dir, dict_reuse_map={}, model_type='small', num_tasks=5, num_prefs_per_task=100, num_models_per_pref=1000, alpha=0.5):
    dict_all_accs = {}
    dict_all_prefs = {}
    ...
    for i in range(num_tasks):
        ...
        hdrs = []
        for pref in prefs:
            dists_combined = pref_convex_combination(pref, dict_reuse_map=dict_reuse_map, model_type=model_type)
            hdr = compute_hdr(dists_combined, alpha=alpha)
            hdrs += [hdr]
        ...
        torch.save(dict_all_accs, os.path.join(data_dir, f'dict_all_accs_{alpha}_sublinear.pt'))
...
if __name__ == '__main__':
    ...
    pyro.get_param_store().load(os.path.join(args.data_dir, 'fgcs_sublinear.pth'))

    # Load posterior reuse map for sublinear buffer growth
    dict_reuse_map = torch.load(os.path.join(args.data_dir, 'dict_reuse_map.pt'))
    ...
    _, _ = gen_acc_dict(all_data_test, all_label_test, args.data_dir, dict_reuse_map=dict_reuse_map, model_type=args.model_size, num_tasks=int(num_tasks), num_prefs_per_task=int(args.num_prefs_per_task), num_models_per_pref=int(args.num_models_per_pref), alpha=float(args.alpha))
